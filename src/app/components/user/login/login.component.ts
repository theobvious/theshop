...
export class LoginComponent implements OnInit {
  ...
  logIn(): void {
    ...
    .subscribe(
        ...
        err=> this.message="Something went wrong. Try again or register."
    )
  }

  nextStep() {
    this.cartservice.checkOpenCart()
    .subscribe(res=> {
      if(res.json()=='2') {
        this.router.navigate([this.shopurl]);
      } else {
        sessionStorage.setItem('cartid', res.json());
        this.router.navigate([this.activecarturl]);
      }
    });
  }
}